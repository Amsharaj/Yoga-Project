...
const PendingClass = () => {
    ...
    useEffect(() => {
        axiosSecure.get(`/classes/${currentUser?.email}`)
            .then(res => {
                const approved = res.data.filter(cls => cls.status === 'pending');
                setPendingClasses(approved);
            })
        ...
        <div>
            <div className="my-9">
                ...
                <div className="text-center">
                    <Fade duration={100} className='text-[12px]  text-center' cascade>Here you can see how many classes not approved by admin</Fade>
                </div>

                <div className="">
                    {
                        pendingClasses.length === 0 ? <div className='text-center text-2xl font-bold mt-10'>You have not Pending any class yet</div> :
                            <div className="mt-9">
                                {
